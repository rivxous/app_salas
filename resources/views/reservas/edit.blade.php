                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3" id="salaContainer" style="display: {{ old('fk_idSala') ? 'block' : 'none' }};">
                            ...
                            <select id="fk_idSala" name="fk_idSala" class="form-select" required>
                                ...
                                @if(old('ubicacion', $reserva->sala->ubicacion))
                                    @foreach ($salas->where('ubicacion', old('ubicacion', $reserva->sala->ubicacion)) as $sala)
                                        <option value="{{ $sala->id }}" {{ old('fk_idSala', $reserva->fk_idSala) == $sala->id ? 'selected' : '' }}>
                                            {{ $sala->nombre }}
                                        </option>
                                    @endforeach
                                @endif
                        ...
                        <div id="camposReserva" style="display: {{ old('fk_idSala') ? 'block' : 'none' }};">
                            ...
                            <div class="mb-3">
                                ...
                                {!! Form::select(
                                    ...
                                    old('tipoEvento', $reserva->tipoEvento),
                                    [
                                        'class' => 'form-select',
                                        'id' => 'tipoEvento',
                                        'required' => true,
                            ...
                            <div class="mb-3">
                                ...
                                <div id="fechasContainer">
                                    @foreach(old('fechas', $reserva->reserva_horarios) as $index => $horario)
                                        <div class="row g-3 mb-3 fecha-grupo">
                                            <div class="col-md-4">
                                                {!! Form::label('fechas[]', 'Fecha', ['class' => 'form-label']) !!}
                                                {!! Form::date('fechas[]', \Carbon\Carbon::parse($horario->fecha), ['class' => 'form-control', 'required' => true]) !!}
                                            </div>
                                            <div class="col-md-3">
                                                {!! Form::label('horas_inicio[]', 'Hora de Inicio', ['class' => 'form-label']) !!}
                                                {!! Form::time('horas_inicio[]', \Carbon\Carbon::parse($horario->hora_inicio)->format('H:i'), ['class' => 'form-control', 'required' => true]) !!}
                                            </div>
                                            <div class="col-md-3">
                                                {!! Form::label('horas_fin[]', 'Hora de Fin', ['class' => 'form-label']) !!}
                                                {!! Form::time('horas_fin[]', \Carbon\Carbon::parse($horario->hora_fin)->format('H:i'), ['class' => 'form-control', 'required' => true]) !!}
                                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                {!! Form::select('participantes[]', $usuarios, old('participantes', $reserva->participantes_reservas->pluck('fk_idUsuario')->toArray()), [
                                    'id' => 'fk_participantes',
                                    'class' => 'form-select',
                                    'multiple' => 'multiple',
                                    'style' => 'width: 100%',
    ...
    <script>
        $(document).ready(function () {
            ...
            function inicializarFormulario() {
                if ($('#ubicacion').val()) {
                    ...
                    if (ubicacion && salasPorUbicacion[ubicacion]) {
                        ...
                        salasPorUbicacion[ubicacion].forEach(sala => {
                            const selected = sala.id == '{{ old('fk_idSala', $reserva->fk_idSala) }}' ? 'selected' : '';
                ...
                @if((old('fechas') && count(old('fechas')) > 1) || ($reserva->reserva_horarios->count() > 1))
            ...
            async function cargarReservasEnCalendario(salaId) {
                try {
                    const response = await fetch("{{ route('buscar_salas_horarios_disponibles') }}", {
                        ...
                        body: JSON.stringify({id_sala: salaId, reserva_id: reservaId})
                    });
